  ...
  <div class="vueAnkaCropper">
    <div class="ankaCropper" :class="[opts.skin]">
      ...
      >
        ...
        <button class="ankaCropper__selectButton" @click="triggerInput">
        ...
        >
          <img
            ...
            :style="{
              ...
              borderRadius: opts.cropArea === 'circle' ? '50%' : 0,
            }"
          />
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      ctx: false,
      defaultOptions: {
        aspectRatio: 3 / 1, // false or number, always width / height, locks aspect ratio of cropper. It should equal to croppedWidth / croppedHeight
        closeOnSave: true,
        ...
        croppedWidth: 800, // desired width of cropped image (or false)
        cropperHeight: false,
        dropareaMessage:
          "Arraste e solte uma imagem aqui ou use o botão abaixo.",
  ...
  computed: {
    ...
    cropperHeight() {
      if (
        this.opts.cropperHeight &&
        this.fullWidth > this.opts.layoutBreakpoint
      ) {
        return this.opts.cropperHeight - 80;
      }
    ...
    markers() {
      return {
        nw: {
          x: this.x - this.opts.handleSize,
          y: this.y - this.opts.handleSize,
        },
        ne: {
          x: this.x + this.w - this.opts.handleSize,
          y: this.y - this.opts.handleSize,
        },
        sw: {
          x: this.x - this.opts.handleSize,
          y: this.y + this.h - this.opts.handleSize,
        },
        se: {
          x: this.x + this.w - this.opts.handleSize,
          y: this.y + this.h - this.opts.handleSize,
    ...
    prevdivHeight() {
      if (this.fullWidth > this.opts.layoutBreakpoint)
        return this.cropperHeight;
      return 300;
    },
    prevdivWidth() {
    ...
    previewSize() {
      ...
      let resratio =
        Math.round((this.resultWidth / this.resultHeight) * 1000) / 1000;
      let pw, ph;
      if (resratio > pdratio) {
        pw = dw;
        ph = dw / resratio;
      ...
      return {
        w: Math.min(pw, this.resultWidth),
        h: Math.min(ph, this.resultHeight),
      };
    },
    resultCanvas() {
      if (!this.image) {
    ...
    resultWidth() {
      let [ar, cw, ch] = [
        ...
        this.opts.croppedWidth,
        this.opts.croppedHeight,
      ];
      let imageFactor =
        Math.round((this.imageWidth / this.canvasWidth) * 1000) / 1000;
      let ratio = ar ? ar : this.w / this.h;
      if (cw && !ch) {
        return cw;
      }
      if (!cw && !ch) {
        return Math.round(this.w * imageFactor);
      }
      if (!cw && ch) {
    ...
    resultHeight() {
      ...
      let imageFactor =
        Math.round((this.imageHeight / this.canvasHeight) * 1000) / 1000;
      let ratio = ar ? ar : this.w / this.h;
      if (ch && !cw) {
        return ch;
      }
      if (!cw && !ch) {
        return Math.round(this.h * imageFactor);
      }
      if (!ch && cw) {
  ...
  methods: {
    ...
    doCrop() {
      let resultImage = this.resultCanvas.toDataURL(
        this.opts.resultMimeType,
        this.opts.resultQuality
      );
      ...
      this.resultCanvas.toBlob(
        (blob) => {
          ...
          if (this.opts.uploadTo) {
            ...
            axios.post(this.opts.uploadTo, formData).then((response) => {
    ...
    drawCanvas() {
      ...
      if (
        this.opts.showPreview &&
        this.opts.previewOnDrag &&
        this.previewCanvas
      ) {
        this.previewImage = this.previewCanvas.toDataURL(
          "image/jpeg",
          this.opts.previewQuality
        );
      }
    },
    drawImageOnCanvas() {
    ...
    stopDrag() {
      this.dragged = false;
      let preview = this.resultCanvas.toDataURL(
        "image/jpeg",
        this.opts.previewQuality
      );
      ...
      if (this.opts.maxFileSize && file.size > this.opts.maxFileSize) {
        ...
        this.$emit(
          ...
          "File too large (" +
            fileSize +
            ")! Max file size is " +
            this.humanFileSize(this.opts.maxFileSize)
        );
        return;
      }
      this.file = file;
